...
const COACHES = ["HelvÃ©tia","Romain","Olivier","Yves","Jonathan","Hugo","Fernando","JÃ©rÃ©mie","Michel","Romane","Perso","Autres"];
...
const EMPTY_STAGE = { date: new Date().toISOString().split("T")[0], satisfaction:8, katas:[], coaches:[], duration:120, focus:"", corrections:"", retours:"" };

const EDF_COACHES = ["Yves B","Yves R","Fabien","Jean-Luc","Lucas"];
const StageEquipe = () => {
  ...
  const handleSave = async () => {
    ...
    // firestore_edf â persistance Firestore
    if (!editingId) {
      try {
    ...
    // notif_edf â notification push creation Stage EDF
    notifyNewContent({
          ...
          <div style={{ background:"#fff", width:"100%", maxHeight:"92vh", overflowY:"auto", borderRadius:"20px 20px 0 0" }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ padding:"20px 24px" }}>
              ...
              <MultiSelect label="Katas pratiquÃ©s" options={KATAS_LIST} selected={form.katas||[]}
                ...
                onRemove={v=>setForm(f=>({...f,katas:(f.katas||[]).filter(k=>k!==v)}))}
                color="#1D4ED8" />
              <MultiSelect label="Entraîneur(s)" options={EDF_COACHES} selected={form.coaches||[]}
                onAdd={v => setForm(f => ({...f, coaches:[...(f.coaches||[]),v]}))}
                onRemove={v => setForm(f => ({...f, coaches:(f.coaches||[]).filter(c => c!==v)}))}
                color="#059669" />
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      unsub = onSnapshot(collection(db, "seances"), (snap) => {
        setSessions(snap.docs.map(d => ({ ...d.data(), id: d.id })).filter(s => s.type !== 'stage_edf'));
      });
    };
    init();
    return () => unsub && unsub();
